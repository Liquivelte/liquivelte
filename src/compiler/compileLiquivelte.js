...
function compileComponentPropAttribute(propName, propValue) {
...
export function compileLiquivelte(source, options) {
    ...
    const scriptToken = tokens.find(t => t.type === 'Script');
    ...
    for (const token of tokens) {
        if (token.type === 'Html') {
            ...
            for (const [propName, propValue] of Object.entries(component.props)) {
                svelteCode += compileComponentPropAttribute(propName, propValue);
            }
    ...
    for (const token of tokens) {
        if (token.type === 'Html') {
            ...
            const frame = traceFrames.find((f) => f.type === 'component' && f.module === moduleName);
...
//# sourceMappingURL=compileLiquivelte.js.map