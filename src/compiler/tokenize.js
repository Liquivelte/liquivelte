...
export function tokenize(source, options) {
    ...
    scan: while (pos < source.length) {
        ...
        let nextSpecial = -1;
        if (nextLiquid >= 0)
            nextSpecial = nextLiquid;
        if (nextComponent >= 0 && (nextSpecial < 0 || nextComponent < nextSpecial))
            nextSpecial = nextComponent;
        if (nextScript >= 0 && (nextSpecial < 0 || nextScript < nextSpecial))
            nextSpecial = nextScript;
        if (nextSpecial >= 0) {
            if (nextSpecial > 0) {
                tokens.push({
                    type: 'Html',
